...
$systemText =
    ...
    "OUTPUT FORMAT — return ONLY a single JSON object, no prose, exactly shaped like this:\n" .
    "{\n" .
    "  \"summary\": \"two sentence overview\",\n" .
    "  \"suggestions\": [\n" .
    "    {\n" .
    "      \"name\": \"Plant name\",\n" .
    "      \"species\": \"Latin name or empty string\",\n" .
    "      \"type\": \"one of indoor|outdoor|succulent|tropical|vegetable|flowering|herb\",\n" .
    "      \"encyclopedia_id\": 123,\n" .
    ...
    "  ]\n" .
    "}\n" .
    "Use encyclopedia_id null when the plant is not in the catalogue. The \"suggestions\" array must contain exactly {$count} items.";
...
try {
    ...
    gdRespond(false, $e->getMessage(), null, 500);
}

$clean = preg_replace('/^```(?:json)?\s*|\s*```$/m', '', $text);
$parsed = json_decode($clean, true);
if (!is_array($parsed)) {
    // Fallback: pull the outermost { ... } block in case of stray wrapping text.
    $s = strpos($clean, '{');
    $e = strrpos($clean, '}');

...
const Login = ({ onSwitchToRegister, onLogin }) => {
  ...
  const handleReset = async () => {
    setResetMsg(null);
    if (!resetEmail || !resetPass) { setResetMsg({ type: 'error', text: 'Enter your email and a new password.' }); return; }
    if (resetPass.length < 6)      { setResetMsg({ type: 'error', text: 'Password must be at least 6 characters.' }); return; }
    setResetting(true);
    try {
      const res = await apiService.resetPassword(resetEmail.trim(), resetPass);
      if (res.success) {
        setResetMsg({ type: 'ok', text: 'Password updated — you can sign in now.' });
        setResetPass('');
        setTimeout(() => setResetMode(false), 1500);
      } else {
        setResetMsg({ type: 'error', text: res.message || 'Could not reset password.' });
      }
    } catch (e) {
      setResetMsg({ type: 'error', text: e.message || 'Could not reset password.' });
    } finally {
      setResetting(false);
    }
  };
          ...
          <form onSubmit={handleSubmit} className="auth-form" noValidate>
            ...
            {resetMode && (
              <div className="reset-panel">
                <p className="reset-hint">Enter your email and a new password to reset it.</p>
                <div className="input-wrap">
                  <i className="fas fa-envelope input-icon"></i>
                  <input
                    type="email"
                ...
                {resetMsg && (
                  <span className={resetMsg.type === 'ok' ? 'reset-ok' : 'field-error'} style={{ marginTop: 6, display: 'block' }}>
                    {resetMsg.text}
                  </span>
                )}
                <button

...
function callGeminiVision($imagePath, $ext, $userNotes, $plantContext) {
    ...
    // Ask for the exact JSON shape we want (Groq has no server-side schema, so spell it out).
    $systemText .= "\n\nReturn ONLY a JSON object with these keys: " .
        "\"summary\" (string, one-line headline max 80 chars), " .
        "\"severity\" (one of: healthy, mild, moderate, severe, unknown), " .
        "\"confidence\" (integer 0-100), " .
        "\"diagnosis\" (string, 2-4 sentences), " .
        "\"recommendations\" (string of 3-6 bullet points, each on its own line prefixed with '- ').";

    $dataUrl = 'data:' . $mime . ';base64,' . $b64;
    $text = groqVision($systemText, $userText, $dataUrl, [
        'temperature' => 0.4,
        'max_tokens'  => 1024,
        'json'        => true,
        'timeout'     => 60,

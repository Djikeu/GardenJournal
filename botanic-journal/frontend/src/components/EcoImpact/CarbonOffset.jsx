              ...
              {breakdown.map(p => {
                ...
                return (
                  <div key={p.id} className="co-plant-row">
                    {(p.image_url || p.image) ? (
                      <img src={buildImage(p.image_url || p.image)} alt={p.name} onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'flex'; }} />
                    ) : null}
                    <div className="img-fallback" style={{ display: (p.image_url || p.image) ? 'none' : 'flex' }}>
                      <i className="fas fa-seedling"></i>
                    </div>
                    <div className="info">

...
const PlantDetail = ({ showNotification, user, plantId, onClose, onBack }) => {
    ...
    const addToCollection = async () => {
        ...
        try {
            ...
            const plantData = {
                ...
                description: cleanText(plant.description) || '',
    ...
    const getSoilType = (plant) => {
        ...
        if (cleanText(plant.care_instructions)) {
            if (cleanText(plant.care_instructions).toLowerCase().includes('well-draining')) return 'Well-draining potting mix';
        }
        return 'Standard potting mix';
    };

    const getToxicity = (plant) => {
        if (cleanText(plant.care_instructions) && cleanText(plant.care_instructions).toLowerCase().includes('toxic')) {
            ...
            <div className="plant-detail-container">
                ...
                <div className="plant-header-section">
                    <div className="plant-image-container">
                        ...
                        <div className="img-fallback" style={{ display: (plant.image_url || plant.image) ? 'none' : 'flex' }}>
                            <i className="fas fa-seedling"></i>
                        </div>
                    ...
                    <div className="plant-info-section">
                        ...
                        <div className="plant-description-preview">
                            <p>{cleanText(plant.description) || 'A beautiful plant that adds life to any space. Perfect for both beginners and experienced plant enthusiasts.'}</p>
                        </div>
                ...
                <div className="tab-content">
                    {activeTab === 'overview' && (
                        <div className="overview-content">
                            <div className="detail-cards-grid">
                                <div className="detail-card">
                                    ...
                                    <div className="card-body">
                                        ...
                                        <p className="requirement-desc">
                                            {cleanText(plant.care_instructions)
                                                ? cleanText(plant.care_instructions).split('.')[0] + '.'
                                                : (plant.type === 'indoor'
                                ...
                                <div className="detail-card">
                                    ...
                                    <div className="card-body">
                                        ...
                                        <p className="requirement-desc">
                                            {cleanText(plant.care_instructions)
                                                ? cleanText(plant.care_instructions).split('.')[1] || cleanText(plant.care_instructions).split('.')[0]
                            ...
                            {cleanText(plant.description) && (
                                <div className="full-description">
                                    <h3>About {plant.name}</h3>
                                    <div className="description-content">
                                        <p>{cleanText(plant.description)}</p>
                            ...
                            {cleanText(plant.care_instructions) && (
                                <div className="care-quick-tips">
                                    ...
                                    <div className="care-tips-grid">
                                        {cleanText(plant.care_instructions).split('. ')
                                            .filter(s => s.trim())
                                            .map((tip, idx) => (
                    ...
                    {activeTab === 'tips' && cleanText(plant.care_instructions) && (
                            ...
                            {relatedPlants.map(relatedPlant => (
                                <div key={relatedPlant.id} className="related-plant-card" onClick={() => window.location.hash = `plant/${relatedPlant.id}`}>
                                    <div className="related-plant-image">
                                        {(relatedPlant.image_url || relatedPlant.image) ? (
                                            <img
                                                src={relatedPlant.image_url || relatedPlant.image}
                                                alt={relatedPlant.name}
                                                onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'flex'; }}
                                            />
                                        ) : null}
                                        <div className="img-fallback" style={{ display: (relatedPlant.image_url || relatedPlant.image) ? 'none' : 'flex' }}>
                                            <i className="fas fa-seedling"></i>
                                        </div>

...
// ─────────────────────────────────────────────────────────────────────
// Gemini call — returns one structured detective case
// ─────────────────────────────────────────────────────────────────────
function callGeminiForCase($difficulty, $avoidBlock) {
    ...
    [$body, $code, $err] = httpPostJson($url, $payload);

    if ($body === false)               throw new Exception('Gemini request failed: ' . $err);
    if ($code < 200 || $code >= 300) {
        $parsedErr = json_decode($body, true);
        throw new Exception('Gemini API error: ' . ($parsedErr['error']['message'] ?? ('HTTP ' . $code)));
    }
    ...
    if (!is_array($parsed)) throw new Exception('Gemini returned non-JSON: ' . substr($text, 0, 300));
